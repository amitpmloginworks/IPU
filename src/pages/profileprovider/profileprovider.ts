...
import { IonicPage, NavController, NavParams , ToastController, MenuController, ViewController, LoadingController, AlertController,PopoverController, ActionSheetController, ModalController } from 'ionic-angular';
...
import { ProfilenewitemPage } from '../profilenewitem/profilenewitem'; 
import { NotificationPage } from '../notification/notification';    

/**
 * Generated class for the ProfileproviderPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-profileprovider',
  templateUrl: 'profileprovider.html',
})
export class ProfileproviderPage {
  chooseOptions

  // Profile detail segment
  FirstName
  LastName
  email
  phoneNo
  ...
  GetLoadData(){  
    ...
    this.security.UserViews().subscribe(result => {
        ...
        if(result==false){  
          ...
          var a = result.providing.services;
          this.ServiceArr = a.split(',').map(function(item) {    return item;  }); 
          this.ServiceArrLen=this.ServiceArr.length 
          this.Website=result.providing.website
          if(result.portfolio != undefined){
            this.PortfolioArr=result.portfolio
          }
...
this.SendOTP();
 ...
 SendOTP(){
  ...
  addItem(){ 
    let addItemModal = this.modalCtrl.create(ProfilenewitemPage);  
    addItemModal.onDidDismiss(data => { this.GetLoadData(); this.chooseOptions="portfolio";  }); 
    addItemModal.present();    
  }

  ionViewDidLoad() {
    console.log('ionViewDidLoad ProfileproviderPage');
  }
